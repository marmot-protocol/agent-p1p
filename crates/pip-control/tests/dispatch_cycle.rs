...
fn planner_dispatch_freezes_intent_and_projects_only_worker_then_acks_outbox() {
    ...
    assert!(worker_body.get("immutable_evidence_bundle").is_none());
    let projections = store.unconsumed_task_projections().unwrap();
    let bundle = &projections[0].desired["body"]["immutable_evidence_bundle"];
    assert_eq!(
