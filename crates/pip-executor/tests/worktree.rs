...
fn prepare(tmp: &tempfile::TempDir) -> (PathBuf, PathBuf, WorktreeSpec) {
    ...
    let spec = WorktreeSpec::new(&repository, &root, "pip/v2/", case_id(), base()).unwrap();
...
fn names_are_deterministic_and_remain_under_the_controller_root() {
    ...
    assert_eq!(spec.branch(), "pip/v2/repo-984321/issue-1240/workflow-2");
    assert_eq!(
        spec.path(),
        root.canonicalize()
            .unwrap()
...
fn process_allocator_creates_and_reconciles_a_real_git_worktree() {
    ...
    let spec = WorktreeSpec::new(&repository, &root, "pip/v2/", case_id(), sha).unwrap();
    let allocator =
